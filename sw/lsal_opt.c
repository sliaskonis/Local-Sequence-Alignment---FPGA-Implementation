	...
	int *max_index, int *similarity_matrix, short *direction_matrix, int N, int M)
{
	...
	*max_index = N;
	...
	// First element depends only on similarity between string's first elements
	if(string1[0] == string2[0]) {
		...
		direction_matrix[0] = CENTER;
	}

	// Fill the first row of the similarity matrix, starting from the second element
	for (index = 1; index < N; index++) {
		int match = 0;
		int test_val1 = 0, test_val2;
		int val = 0;
		short dir = 0;

		match = (string1[index-1] == string2[0]) ? MATCH : MISS_MATCH;
	...
	for(i = 1; i < M; i++) {
		for (j = 1; j < N; j++) {
			...
			match = (string1[j-1] == string2[i]) ? MATCH : MISS_MATCH;
...
int main(int argc, char** argv) {
	...
	short *direction_matrix = (short*) malloc(sizeof(short) * (N+1) * M);
	...
	memset(similarity_matrix, 0, sizeof(int) * (N+1) * M);
	memset(direction_matrix, 0, sizeof(short) * (N+1) * M);

	/************************ Execute LSAL Algorithm  ************************/
    t1 = clock();
	compute_matrices(query, database, max_index, similarity_matrix, direction_matrix, N+1, M);
	t2 = clock();
